  ...
  return (
    <div className='h-screen shadow-md p-5'>
      ...
      <div className='mt-10'>
        ...
        <div className='mt-5'>
            {MenuList.map((menu,index)=>(
                <div key={index} className={`flex gap-5 items-center p-3 hover:bg-slate-200 rounded-lg 
                cursor-pointer mt-3 ${path==menu.path&&`bg-slate-200`}`}> 
                <menu.icon />
                <h2>{menu.name}</h2>
                </div>
            ))}
      ...
      <div className='border p-3 bg-slate-100 rounded-lg absolute bottom-10 w-[85%]'>
       ...
       <Link href={'/dashboard/upgrade'} className='text-blue-800 text-xs mt-3'><h2>Upgrade to create more</h2></Link> 
      </div>
    </div>
  )
}
